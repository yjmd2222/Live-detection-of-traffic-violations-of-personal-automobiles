...
def resize(old_image_root, new_image_root, nw, keep_aspect, progress_bar, file):
...
def main(old_path, new_path, nw=1280, keep_aspect=True):
    '이미지 resize'
    ...
    for old_image_root, _, files in os.walk(old_path):
        ...
        with ThreadPoolExecutor() as executor:
            executor.map(lambda file: resize(old_image_root, new_image_root, nw, keep_aspect, progress_bar, file), files)
        progress_bar.close()
