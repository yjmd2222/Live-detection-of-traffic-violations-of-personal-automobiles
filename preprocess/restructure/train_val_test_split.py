...
def input_image_parent_dirs(train_ratio=0.8, val_ratio=0.1):
    '''
    이미지, json, txt 폴더 하위 폴더로 trian, val, test 셋으로 구분\n
    기본 구조 '데이터/images/image0.jpg' 등으로 가정.
    '''
    # 폴더 생성
    for folder in ['images', 'labels_json', 'labels_det', 'labels_seg']:
        for split in ['train', 'val', 'test']:
            os.makedirs(f'데이터/{folder}/{split}', exist_ok=True)

    # 섞기
    image_file_names = list(set(os.listdir('데이터/images')) - {'train', 'val', 'test'})
    np.random.seed(42)
    ...
    # 부모 디렉터리
    old_image_parent_path = '데이터/images'
    old_json_parent_path = '데이터/labels_json'
    old_det_txt_parent_path = '데이터/labels_det'
    old_seg_txt_parent_path = '데이터/labels_seg'
